...
```
  ...
  |                     FastAPI Application                           |
  |  +------------------------------------------------------------+  |
  |  |  Authentication (Azure Entra ID)  |  Rate Limiting (Redis) |  |
  |  +------------------------------------------------------------+  |
...
- **Azure Cache for Redis** -- Caches PubMed API responses to reduce latency and respect rate limits
...
- **Network:** VNet integration with private endpoints for all Azure services
